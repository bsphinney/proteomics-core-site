...
def generate_comprehensive_report(nih_stats, nsf_stats, pubmed_data, sub_stats, cross_refs,
    ...
    lines = [
        ...
        "",
        f"**Source code & data:** [{GITHUB_REPO_URL}]({GITHUB_REPO_URL})",
        "",
    ]
    if report_note:
    ...
    lines.extend([
        ...
        f"| File | Description |",
        f"|------|-------------|",
        f"| [`executive_summary.pdf`]({GITHUB_REPO_URL}/blob/main/reports/executive_summary.pdf) | One-page visual dashboard for leadership |",
        f"| [`impact_report_latest.md`]({GITHUB_REPO_URL}/blob/main/reports/impact_report_latest.md) | This report (comprehensive, all services) |",
        f"| [`impact_report_ms_only_latest.md`]({GITHUB_REPO_URL}/blob/main/reports/impact_report_ms_only_latest.md) | Mass spectrometry only (for S10 grant renewals) |",
        f"| [`impact_report_aaa_only_latest.md`]({GITHUB_REPO_URL}/blob/main/reports/impact_report_aaa_only_latest.md) | Amino acid analysis only |",
        f"| [`core_user_grants_*.csv`]({GITHUB_REPO_URL}/tree/main/reports) | PI-level grant data (backs the funding claims) |",
        f"| [`nih_grants_*.csv`]({GITHUB_REPO_URL}/tree/main/reports) | All NIH grants in spreadsheet form |",
        f"| [`nsf_awards_*.csv`]({GITHUB_REPO_URL}/tree/main/reports) | All NSF awards in spreadsheet form |",
        f"| [`impact_data.json`]({GITHUB_REPO_URL}/blob/main/reports/impact_data.json) | Machine-readable data for website integration |",
        f"| [`figures/*.png`]({GITHUB_REPO_URL}/tree/main/reports/figures) | All charts (9 total) |",
        "",
        "### How to Re-run", "",
        "```bash",
        "pip install requests matplotlib",
